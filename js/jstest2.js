...
var range = $('.input-range'),    value = $('.range-value');    value.html(range.attr('value'));range.on('input', function(){    value.html(this.value);}); 

/* show and hide divs depending on slider choice*/
$('.input-range').on('change', function(){
    var value = $(this).val();
    if(value >= 250 ) {
      $("#chooseplan").show(1000);
    } else if (value < 250){
      $("#chooseplan").hide(1000);
    }
});